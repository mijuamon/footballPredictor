...
public class FileLoaderUtil {
    ...
    private static List<TeamModel> connectAll(List<TeamDTO> teamsDTO, List<PlayerDTO> playersDTO, List<ScoreDTO> scoreDTO, List<MatchDTO> matchsDTO) {
        ...
        for (final PlayerDTO player : playersDTO) {
            ...
            if (teams.stream().anyMatch(x -> x.getID().equals(player.getTeamID()))) {
                final TeamModel t = teams.stream().filter(x -> player.getTeamID().equals(x.getID())).findFirst().get();
                final List<ScoreModel> scoresList = scoreDTO.stream().filter(x -> player.getID().equals(x.getPlayerID())).map(x -> convert(x)).collect(Collectors.toList());
        ...
        for (final MatchDTO match : matchsDTO) {
            ...
            if (teams.stream().anyMatch(x -> x.getID().equals(match.getLocalId())) && teams.stream().anyMatch(x -> x.getID().equals(match.getVisitorId()))) {
        ...
        for (final ScoreDTO score : scoreDTO) {
            final PlayerModel currentPlayer = players.stream().filter(p -> p.getID().equals(score.getPlayerID())).findFirst().get();
            final MatchModel currentMatch = matchs.stream().filter(m -> m.getID().equals(score.getMatchID())).findFirst().get();
