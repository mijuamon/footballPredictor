...
public class Converters {
    ...
    {
        return new PlayerModel(source.getID(), source.getName());
    }

    public static TeamModel convert (TeamDTO source)
    {
        return new TeamModel(source.getID(), source.getName());
    }
    public static ScoreModel convert (ScoreDTO source)
    {
        return new ScoreModel(source.getID(),source.getScore());
    }
    public static MatchModel convert (MatchDTO source)
    {
        return new MatchModel(source.getID(),source.getYear(), source.getJourney(), source.getResult());
    }
}
